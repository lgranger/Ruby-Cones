...
# --------------------------------------------------------------------
# Support code for the Ruby Cones.
# --------------------------------------------------------------------
...
module Neo
  ...
  class Mechanic
    ...
    PROGRESS_FILE_NAME = '.road_progress'
    ...
    def observe(step)
      car_problem = ["has given you a flat tire.", "is making smoke pouring our of you hood.", "and so you've run out of gas.", "made the timing belt break.", "is a cop that has pulled you over for not wearing a seatbelt."]
      if step.passed?
        @pass_count += 1
        if @pass_count > progress.last.to_i
          @observations << Color.green("#{step.cone_file}##{step.name} has moved you down the road.")
        end
      else
        @failed_test = step
        @failure = step.failure
        add_progress(@pass_count)
        @observations << Color.red("#{step.cone_file}##{step.name} #{car_problem[rand(car_problem.length)]}.")
        throw :neo_exit
      end
    end
    ...
    def show_progress
      bar_width = 50
      total_tests = Neo::Cone.total_tests
      scale = bar_width.to_f/total_tests
      print Color.green("your distance down the road [")
    ...
    def boring_end_screen
      puts "\"We travel not to escape life, but for life to not escape us\""
    end
    ...
    def artistic_end_screen
      ...
      ruby_version = ruby_version.side_padding(54)
        completed = <<-ENDTEXT
             ...
             /                                              \\
            /                  adapted from                  \\
           /         Neo Software Artisans "Ruby Koans"       \\
          /                                                    \\
         /=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\\
        /=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\\
   ____/=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\\____
...
ENDTEXT
    ...
    def encourage
      puts
      puts "Mechanic Bob says:"
      puts Color.cyan("  You're not to the finish line yet!")
      if ((recents = progress.last(5)) && recents.size == 5 && recents.uniq.size == 1)
        puts Color.cyan("  I can see you're feeling stuck, don't be afraid to ask for help.")
      elsif progress.last(2).size == 2 && progress.last(2).uniq.size == 1
        puts Color.cyan("  You got this! Don't lose hope.")
      elsif progress.last.to_i > 0
        puts Color.cyan("  You're making good time! Wa-hoo! #{progress.last} completed.")
      end
    end

    def guide_through_error
      puts
      puts "Getting that engine running is gonna need..."
      puts Color.red(indent(failure.message).join)
      puts
      puts "Take a look at this part over here:"
    ...
    def a_travel_statement
      if !failed?
        travel_statement =  "\"We travel not to escape life, but for life to not escape us\""
      else
        travel_statement = case (@pass_count % 10)
        when 0
        ...
        when 7, 8
          "\"To travel is to live\" -Hans Christan Andersen"
        else
          "\"We travel not to escape life, but for life to not escape us\""
        end
      end
      puts Color.green(travel_statement)
    end
  end
  ...
  class Cone
    ...
    attr_reader :name, :failure, :cone_count, :step_count, :cone_file

    def initialize(name, cone_file=nil, cone_count=0, step_count=0)
      @name = name
      @failure = nil
    ...
    def repair
      ...
      begin
        send(name)
      rescue StandardError, Neo::Mechanic::FailedAssertionError => ex
        failed(ex)
      ensure
        begin
          teardown
    ...
    class << self
      ...
      def method_added(name)
        testmethods << name if !tests_disabled? && Cone.test_pattern =~ name.to_s
      end
  ...
  class TheRoad
    def trip
      mechanic = Neo::Mechanic.new
      each_step do |step|
        mechanic.observe(step.repair)
      end
      mechanic.instruct
    end
    ...
    def each_step
        ...
        step_count = 0
        Neo::Cone.subclasses.each_with_index do |cone,cone_index|
          cone.testmethods.each do |method_name|
            step = cone.new(method_name, cone.to_s, cone_index+1, step_count+=1)
            yield step
          end
        end
      }
...
END {
  Neo::Cone.command_line(ARGV)
  Neo::TheRoad.new.trip
}
