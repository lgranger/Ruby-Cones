namespace "check" do
  ...
  task :abouts do
    ...
    about_requires = `grep require src/road_to_victory.rb | wc -l`.to_i
    puts "Checking road_to_victory completeness"
    puts "# of about files:    #{about_files}"
    puts "# of about requires: #{about_requires}"
    if about_files > about_requires
      puts "*** There seems to be requires missing in the road to victory"
    else
      puts "OK"
    end
    puts
